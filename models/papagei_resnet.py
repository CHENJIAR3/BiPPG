...
class ResNet1DMoE(nn.Module):
    ...
    def __init__(self, in_channels, base_filters, kernel_size, stride, groups, n_block, n_classes,
        ...
        # Mixture of Experts (MoE) Head 1 (mt_regression 1)
        self.expert_layers_3 = nn.ModuleList([
            nn.Sequential(
                nn.Linear(out_channels, out_channels // 2),
                nn.ReLU(),
                nn.Linear(out_channels // 2, 1)
        ...
        self.gating_network_3 = nn.Sequential(
            nn.Linear(out_channels, self.n_experts),
            nn.Softmax(dim=1)  # Softmax to produce weights for the experts
        )

        # Mixture of Experts (MoE) Head 2 (mt_regression 2)
        self.expert_layers_4 = nn.ModuleList([
            nn.Sequential(
                nn.Linear(out_channels, out_channels // 2),
                nn.ReLU(),
                nn.Dropout(0.3),
        ...
        self.gating_network_4 = nn.Sequential(
            nn.Linear(out_channels, self.n_experts),
            nn.Softmax(dim=1)  # Softmax to produce weights for the experts
        )
    def forward(self, x):
        ...
        # Mixture of Experts (MoE) Head 1 (mt_regression 1)
        expert_outputs_3 = torch.stack([expert(out) for expert in self.expert_layers_3],
                                       dim=1)  # (batch_size, n_experts, 1)
        gate_weights_3 = self.gating_network_1(out)  # (batch_size, n_experts)
        out_moe3 = torch.sum(gate_weights_3.unsqueeze(2) * expert_outputs_3, dim=1)  # Weighted sum of experts

        # Mixture of Experts (MoE) Head 2 (mt_regression 2)
        expert_outputs_4 = torch.stack([expert(out) for expert in self.expert_layers_4],
                                       dim=1)  # (batch_size, n_experts, 1)
        gate_weights_4 =  self.gating_network_4(out)  # (batch_size, n_experts)
        out_moe4 = torch.sum(gate_weights_4.unsqueeze(2) * expert_outputs_4, dim=1)  # Weighted sum of experts

        outs = torch.cat([out_moe1,out_moe2,out_moe3,out_moe4],dim=1)
        return outs
...
def myResNet1DMoE(args):
    ...
    model_config = {
        ...
        'groups': 1,
        'n_block': 18,
        'n_classes': args.out_dim,  # Embedding dimension
        'n_experts': 3
    }
